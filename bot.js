...
const deleteMessageAfterTimeout = async (chatId, messageId, timeout) => {
  setTimeout(async () => {
    try {
      ...
      console.log(`Message with ID ${messageId} deleted from chat ${chatId}.`);
    } catch (error) {
...
bot.on('document', async (msg) => {
  ...
  if (isBatchActive && isOwner(msg.from.id)) {
    ...
    const singleFile = new FileModel({ uniqueId, ...file, createdBy: msg.from.id });

    await singleFile.save();
    const shareLink = `https://t.me/Hjstreambot?start=${uniqueId}`;
    bot.sendMessage(msg.chat.id, `File saved! Shareable link: ${shareLink}`);
  }
});
...
bot.on('video', async (msg) => {
  ...
  if (isBatchActive && isOwner(msg.from.id)) {
    ...
    await singleFile.save();
    const shareLink = `https://t.me/Hjstreambot?start=${uniqueId}`;
    bot.sendMessage(msg.chat.id, `Video saved! Shareable link: ${shareLink}`);
  }
});
...
bot.on('audio', async (msg) => {
  ...
  if (isBatchActive && isOwner(msg.from.id)) {
    ...
    await singleFile.save();
    const shareLink = `https://t.me/Hjstreambot?start=${uniqueId}`;
    bot.sendMessage(msg.chat.id, `Audio saved! Shareable link: ${shareLink}`);
  }
});
...
bot.onText(/\/finishbatch/, async (msg) => {
  ...
  if (isBatchActive && batchFiles.length > 0) {
    ...
    const batchData = new BatchModel({ batchId: currentBatchId, files: batchFiles, createdBy: telegramId });
    await batchData.save();

    // Generate the shareable link
    const shareLink = `https://t.me/${botUsername}?start=${currentBatchId}`;
    bot.sendMessage(msg.chat.id, `Batch saved successfully! Shareable link: ${shareLink}`);
    ...
    bot.sendMessage(msg.chat.id, 'No active batch or no files have been added.');
  }
});
...
bot.onText(/\/start(.*)/, async (msg, match) => {
  ...
  const imageUrl = 'https://cdn.glitch.global/c29b4992-f073-4c7d-979d-df62eb5b6770/start_image?v=1741520402342'; 
  ...
  if (payload) {
    ...
    if (fileData) {
      if (fileData.fileId) {
        // Send a single file
        const sentMessage = await bot.sendDocument(msg.chat.id, fileData.fileId, { 
        ...
        deleteMessageAfterTimeout(msg.chat.id, sentMessage.message_id, 600000);
      } else if (fileData.files) {
        // If it's a batch of files, send each one
        for (const file of fileData.files) {
          const sentMessage = await bot.sendDocument(msg.chat.id, file.fileId, { caption: file.caption || file.fileName });
          deleteMessageAfterTimeout(msg.chat.id, sentMessage.message_id, 600000);
        }
...
`;
  const aboutMessage = `
<blockquote><b>🎥 Mʏ Nᴀᴍᴇ: <a href='https://t.me/hivajoycloud1bot'>Hivajoy Cloud Bot</a></b></blockquote>
...
bot.onText(/\/users/, async (msg) => {
  ...
  // Check if the user is the owner
  if (!isOwner(telegramId)) {
    return bot.sendMessage(msg.chat.id, 'Only the owner can use this command.');
  }
...
bot.onText(/\/broadcast/, async (msg) => {
  ...
  // Check if the command is a reply to a message or media
  if (msg.reply_to_message) {
    const broadcastMessage = msg.reply_to_message;
    const broadcastType = broadcastMessage.photo ? 'photo' : broadcastMessage.video ? 'video' : 'text';
    ...
    for (const user of users) {
      try {
        if (broadcastType === 'photo') {
          await bot.sendPhoto(user.telegramId, broadcastMessage.photo[0].file_id, { caption: broadcastMessage.caption });
        } else if (broadcastType === 'video') {
          await bot.sendVideo(user.telegramId, broadcastMessage.video.file_id, { caption: broadcastMessage.caption });
        } else {
          await bot.sendMessage(user.telegramId, broadcastMessage.text);
        }
        sentCount++;
      } catch (err) {
        failedCount++;
    ...
    // Reply with the broadcast result
    bot.sendMessage(msg.chat.id, `Broadcast complete!\n✅ Sent to: ${sentCount} users\n❌ Failed to send to: ${failedCount} users.`);
  } else {
    bot.sendMessage(msg.chat.id, 'Please reply to a message or media that you want to broadcast.');
  }
});
